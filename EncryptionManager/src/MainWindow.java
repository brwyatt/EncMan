...
public class MainWindow extends JFrame implements ActionListener, WindowListener{
	...
	private JPanel main;
	private JLabel idLbl=new JLabel("<html><b>Current ID:</b></html>");
	...
	private JPanel createMainPage(){
		...
		JLabel splash;
		try{
			splash=new JLabel(new ImageIcon(Toolkit.getDefaultToolkit().getImage(getClass().getResource("Splash.jpg"))),JLabel.CENTER);
		}catch(Exception e){
			splash=new JLabel("");
		}
		JLabel emVer=new JLabel("<html><b>Encryption Manager version:</b> "+EncryptionManager.VERSION+"</html>");
		JLabel cryptoVer=new JLabel("<html><b>Crypto Library version:</b> "+Crypto.VERSION+"</html>");
		pane.add(splash);
		pane.add(emVer);
		pane.add(cryptoVer);

		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.NORTH, splash, 5, SpringLayout.NORTH, pane);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.WEST, splash, 0, SpringLayout.WEST, pane);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.EAST, splash, 0, SpringLayout.EAST, pane);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.WEST, emVer, 5, SpringLayout.WEST, pane);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.NORTH, emVer, 5, SpringLayout.SOUTH, splash);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.WEST, cryptoVer, 5, SpringLayout.WEST, pane);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.NORTH, cryptoVer, 5, SpringLayout.SOUTH, emVer);
