...
public class FirstRunWizard extends JFrame implements ActionListener {
	...
	final int lastPage=6;//Last page
	...
	JCheckBox startApp=new JCheckBox("Start Encryption Manager",true);
	...
	public void actionPerformed(ActionEvent ae){
		if(ae.getSource()==cancelBtn){
			...
			if(page>lastPage){//if the finish btn is clicked
				...
				dispose();
				if(startApp.isSelected()){
					String tmp="";
					try {
						tmp=ClassLoader.getSystemResource("EncryptionManager.class").toString();
						tmp = tmp.substring(tmp.indexOf("file:/") + 6, tmp.indexOf("!/"));
						try {
							Runtime.getRuntime().exec("java -jar "+tmp.substring(tmp.lastIndexOf("/")+1), null, new File(tmp.substring(0,tmp.lastIndexOf("/")+1).replaceAll("%20", " ")));
						} catch (IOException e) {
							JOptionPane.showMessageDialog(this, "Unable to start application.\n"+e.getMessage(), "Operation Failed", JOptionPane.ERROR_MESSAGE);
						}
					} catch (IndexOutOfBoundsException e) {//not in a jar file
						tmp = tmp.substring(tmp.indexOf("file:/") + 6);
						try {
							Runtime.getRuntime().exec("java "+tmp.substring(tmp.lastIndexOf("/")+1,tmp.lastIndexOf(".")), null, new File(tmp.substring(0,tmp.lastIndexOf("/")+1).replaceAll("%20", " ")));
						} catch (IOException e2) {
							JOptionPane.showMessageDialog(this, "Unable to start application.\n"+e2.getMessage(), "Operation Failed", JOptionPane.ERROR_MESSAGE);
						}
					}
				}else{
					System.exit(0);
				...
				try {
					String tmp=ClassLoader.getSystemResource(this.getClass().getName()+".class").toString();
	...
	private JPanel page0(){//creates page 0
		...
		pane.add(text1);
		JLabel text2=new JLabel("<html>This wizard will help guide you through the setup and configuration of <br> the Encryption Manager.<br><br>If you need help or want more information, click the '?' button below.</html>");//add row2 of text
	...
	private JPanel page2(){//creates page 2
		...
		JLabel info=new JLabel("<html>If you would like to run this program from another location instead of<br>it's current location, select the desired location above.</html>");
		...
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.WEST, info, 5, SpringLayout.WEST, pane);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.NORTH, info, 20, SpringLayout.SOUTH, locfolder);
	...
	private JPanel page3(){//creates page 3
		...
		JButton browseBtn=new JButton("Browse...");
		JLabel infolbl=new JLabel("<html>The Crypto Library provides all the Encryption and Decryption functions<br>required by the Encryption Manager.<br><br>Select the Crypto Library Jar file you wish to use.</html>");
		...
		JEditorPane downloadlbl=new JEditorPane("text/html","<html><a href=\""+EncryptionManager.libDownloadURL+"\">Click here</a> to download the latest version of the Library.</html>");
		...
		downloadlbl.addHyperlinkListener(new HyperlinkListener(){   
			public void hyperlinkUpdate(HyperlinkEvent hle) {
				/*if(EncryptionManager.disableFeatures){
					...
					if (HyperlinkEvent.EventType.ACTIVATED.equals(hle.getEventType())&&JOptionPane.YES_OPTION==JOptionPane.showConfirmDialog(parent, "This action will open a browser window to download the Crypto Library.\n\nDo you wish to continue?", "Continue?", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE)){
						try {
							...
							JOptionPane.showMessageDialog(parent, "Unable to open a browser window!\n\nPlease visit "+EncryptionManager.libDownloadURL+" to manually download the Library.", "Browser Error", JOptionPane.ERROR_MESSAGE);
						}
					}   
				}
			//}
	...
	private JPanel page4(){//creates page 4
		...
		JLabel info=new JLabel("<html>Select the folder in which to store IDs. If the selected folder already<br>contains IDs they will be able to be used.<br><br>The default should work best for most cases.</html>");
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.WEST, liblbl, 5, SpringLayout.WEST, pane);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.NORTH, liblbl, 20, SpringLayout.SOUTH, title);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.WEST, libfolder, 5, SpringLayout.EAST, liblbl);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.NORTH, libfolder, 0, SpringLayout.NORTH, liblbl);
		...
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.NORTH, browseBtn, -4, SpringLayout.NORTH, libfolder);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.WEST, info, 5, SpringLayout.WEST, pane);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.NORTH, info, 20, SpringLayout.SOUTH, libfolder);
		pane.add(liblbl);
		pane.add(libfolder);
		pane.add(browseBtn);
		pane.add(info);
	...
	private JPanel page6(){//creates page 6
		JPanel pane = new JPanel(new SpringLayout());
		JLabel text1=new JLabel("Setup Complete!");//add title (row1) text
		text1.setFont(new Font("",Font.BOLD,15));
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.WEST, text1, 5, SpringLayout.WEST, pane);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.NORTH, text1, 5, SpringLayout.NORTH, pane);
		pane.add(text1);
		JLabel text2=new JLabel("<html>The setup of the Encryption Manager is complete.<br>Click Finish to save the settings and start the program.</html>");//add row2 of text
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.WEST, text2, 10, SpringLayout.WEST, pane);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.NORTH, text2, 25, SpringLayout.SOUTH, text1);
		pane.add(text2);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.WEST, startApp, 10, SpringLayout.WEST, pane);
		((SpringLayout)pane.getLayout()).putConstraint(SpringLayout.SOUTH, startApp, -5, SpringLayout.SOUTH, pane);
		pane.add(startApp);
		nextBtn.setEnabled(true);
		return pane;
	}
	...
	public void actionPerformed(ActionEvent ae){
		if(ae.getSource()==browse){
			...
			if(check.isSelected()){
				...
				if(ClassLoader.getSystemResource(this.getClass().getName()+".class").toString().indexOf("!/")<=0){
